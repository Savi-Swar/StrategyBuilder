...
def sma200_strategy(ticker):
    """Runs the SMA200 Strategy on the selected ticker."""
    print(f"\n🚀 Running SMA200 Strategy for {ticker}...")
    ...
    store_results(ticker, "SMA200", total_return, sharpe_ratio, max_drawdown, win_rate)

    # ✅ Store daily returns
    daily_returns_list = list(zip(df.index, df["Daily Return"]))
    store_daily_returns_unified(ticker, "SMA200", daily_returns_list, RESULTS_DB_PATH)

    print("📊 SMA200 Strategy Results")
    print("───────────────────────────────────────────────")
