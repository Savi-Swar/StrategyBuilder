...
def store_daily_returns_unified(ticker, strategy_name, daily_returns_list, db_path):
    """Stores daily returns in a unified table, overwriting previous entries for same ticker+strategy."""
...
def momentum_strategy(ticker):
    """Runs the Momentum Strategy on the selected ticker."""
    print(f"\n🚀 Running Momentum Strategy for {ticker}...")
    ...
    store_results(ticker, "Momentum", total_return, sharpe_ratio, max_drawdown, win_rate)

    # ✅ Store daily returns
    daily_returns_list = list(zip(df.index, df["Daily Return"]))
    store_daily_returns_unified(ticker, "Momentum", daily_returns_list, RESULTS_DB_PATH)
