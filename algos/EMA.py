...
def get_stock_data(ticker):
    """Fetches stock data from Quark.db instead of quant.db."""
...
def store_results(ticker, strategy_name, total_return, sharpe_ratio, max_drawdown=None, win_rate=None):
    """Stores strategy results in quant.db, only saving available values."""
...
def store_daily_returns_unified(ticker, strategy_name, daily_returns_list, db_path):
    """Stores daily returns in a unified table called daily_returns_table with full overwrite."""
    ...
    cursor.executemany(
        f"INSERT INTO {table_name} (date, ticker, strategy, return) VALUES (?, ?, ?, ?)",
        rows_to_insert
    )
...
def ema_crossover_strategy(ticker):
    print(f"\n🚀 Running EMA Crossover Strategy for {ticker}...")
    ...
    store_results(ticker, "EMA Crossover", total_return, sharpe_ratio, max_drawdown, win_rate)

    # Store daily returns in unified table
    store_daily_returns_unified(ticker, "EMA Crossover", list(zip(df.index, df["Daily Return"])), RESULTS_DB_PATH)

    print("📊 EMA Crossover Strategy Results")
    print("───────────────────────────────────────────────")
