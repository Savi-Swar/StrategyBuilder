...
def rsi_reversal_strategy(ticker):
    """Runs the RSI Reversal Strategy on the selected ticker."""
    print(f"\n🚀 Running RSI Reversal Strategy for {ticker}...")
    ...
    df.loc[(df["RSI"] >= 40) & (df["RSI"] <= 60), "Signal"] = 0
    ...
    store_results(ticker, "RSI Reversal", total_return, sharpe_ratio, max_drawdown, win_rate)

    # ✅ Store Daily Returns
    daily_returns_list = list(zip(df.index, df["Daily Return"]))
    store_daily_returns_unified(ticker, "RSI Reversal", daily_returns_list, RESULTS_DB_PATH)

    print("📊 RSI Reversal Strategy Results")
    print("───────────────────────────────────────────────")
