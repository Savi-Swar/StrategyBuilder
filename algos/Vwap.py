...
def store_results(ticker, strategy_name, total_return, sharpe_ratio, max_drawdown=None, win_rate=None):
    """Stores strategy results in quant.db, handling missing values."""
...
def store_daily_returns_unified(ticker, strategy_name, daily_returns_list, db_path):
    """Stores daily returns in a unified table with overwrite per ticker+strategy."""
    ...
    cursor.executemany("""
        INSERT INTO daily_returns_table (date, ticker, strategy, return)
        VALUES (?, ?, ?, ?)
    """, rows_to_insert)
...
def vwap_strategy(ticker):
    """Runs the VWAP Strategy on the selected ticker."""
    print(f"\n🚀 Running VWAP Strategy for {ticker}...")
    ...
    if "volume" not in df.columns or df["volume"].isna().all() or df["volume"].sum() == 0:
        print(f"⚠️ {ticker} has no usable volume data. Skipping...")
        return
    ...
    store_results(ticker, "VWAP", total_return, sharpe_ratio, max_drawdown, win_rate)

    # Store daily returns
    daily_returns_list = list(zip(df.index, df["Daily Return"]))
    store_daily_returns_unified(ticker, "VWAP", daily_returns_list, RESULTS_DB_PATH)

    if total_return is None or sharpe_ratio is None:
        print(f"⚠️ No valid stats to display for {ticker}.")
        return

    print("📊 VWAP Strategy Results")
    print("───────────────────────────────────────────────")
    print(f"💰 Total Return:       {total_return:.2f}% 📈 (Strategy Performance)")
    print(f"⚡ Sharpe Ratio:       {sharpe_ratio:.2f}  (Higher is better, >1.0 is good)")
    if win_rate is not None:
        print(f"✅ Win Rate:           {win_rate:.2f}%  (Percentage of profitable days)")
    if max_drawdown is not None:
        print(f"🔻 Max Drawdown:       {max_drawdown:.2f}%  (Biggest % loss from peak)")
    print("───────────────────────────────────────────────\n")

    if sharpe_ratio is not None and sharpe_ratio < 0.5:
        print("⚠️ WARNING: Low Sharpe Ratio! Strategy may be risky. Consider optimizing.\n")
    if max_drawdown is not None and max_drawdown < -50:
        print("❌ DANGER: Huge Drawdown! This strategy could **wipe out** your portfolio.\n")

# ✅ Run VWAP Strategy on multiple stocks
tickers = [
    # FX Pairs
