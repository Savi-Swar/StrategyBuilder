# Fees, and the arbitrage that survives them
...
- **Kalshi**: taker ~0.07·p(1−p) per contract, max 1.75c at 50c; July 7, 2026 schedule keeps 0.07 and adds a per-contract multiplier defaulting to 1 [partly confirmed — the primary PDF was unreachable from this network (TLS interception); numbers rest on convergent secondaries and the long-standing published schedule; the ceil-vs-round detail unresolved]. Maker fees 0% on most markets, select markets excepted (help center confirmed; exact excepted-market maker rate conflicts across sources, and one secondary now cites a 0.0175 maker coefficient on some markets — verify against the primary PDF before relying). Pays variable APY on cash **and** open positions (3.25% per help center; daily accrual, monthly payment, $250 minimum — verified; later 3.75–4.05% figures [unconfirmed]), making long-dated full-set lockup positive-carry. Buy-the-field on mutually-exclusive events, executed maker-side, is near-fee-free (probable). ~$31B June volume — the largest venue (verified via PYMNTS/Sportico).
- **Polymarket US** (QCX, CFTC): schedule effective Jul 1, 2026 — taker 0.06·C·p(1−p) (max $1.50/100), **maker rebate Θ = −0.0125 paid at execution** (max $0.31/100 at p=0.5), plus 10/25/50% monthly taker-volume rebates above $250k/$1M/$10M, weekly payout, banker's rounding (docs.polymarket.us/fees, every number verified). The only surface where passive fills earn cash at execution rather than via a daily pool. Requires US KYC; listing mix skews sports (probable). $3.5B June volume (verified).
- **ForecastEx/IBKR**: flat $0.01/contract (YES+NO bids sum to $1.01), cheapest CFTC fee, plus a monthly incentive coupon passing collateral interest to position holders [partly confirmed — mechanism verified via IBKR Campus/secondaries; the 3.13–3.83% APY range unconfirmed; note its economic ladders are typically threshold pairs, **not** strictly mutually-exclusive buckets — the neg-risk-equivalence claim was flagged questionable in verification]. Thin books.
- **Rothera** (Robinhood/SIG JV, live June 2026, ~$2B first month — verified): commissions reportedly capped ~$0.01/contract [unconfirmed — Robinhood fee reports conflict: flat $0.01 vs a k·p(1−p) formula with k=10%; check the primary fee-schedule PDF before modeling].
- **Crypto.com**: fees partially waived when held to resolution ($0.02 open fee on $1 contracts; expiry waives tech fee on $10 contracts) — probable, consistent secondaries.
- **Railbird/DraftKings**: DCM approved, acquired Oct 2025, first sports contracts filed, no fee schedule yet (verified) — watch, don't build.
- **Not viable**: Drift BET (~$295M April 2026 exploit, rebranded Velocity, private beta — verified including live redirect check); Myriad (~$3.9k fees/30d, US restricted to play-money — verified negligible); Limitless (attractive fee model, US-barred by ToS — verified); Paradex (Jan 2026 $0-price liquidation incident — probable red flag).
...
## Competition after the fee wall
...
## Detector/executor spec

Context: fee-aware 60s sweeps already exist in the system. Additions, in build order.

### 1. Maker-side set builder — simulator first (recommended next build)
...
Detector:
- Scan neg-risk events (Gamma API) for ladders where Σ(best NO ask) < K−1−θ (taker check, existing) **and** Σ(best NO bid + 1 tick) < K−1−θ_m (maker-feasible check). θ_m can be far tighter than θ because maker fee = 0 and expected rebate ≈ ρ·r·q(1−q) per filled share is added back per leg.
- Rank candidates by (a) edge after modeled fees, (b) fee-mass concentration 1−Σp_i² (prefer low — tails-heavy), (c) rebateRate (prefer 0.25 categories: Fed/Economics/Politics), (d) resolution-criteria objectivity flag.
- Leg-selection rule from the worked example: rest maker orders on the fat legs (highest q(1−q) fee-mass); take the tail legs where f(p) is near zero. Hybrid beats pure-maker on time-to-completion and pure-taker on cost.

Executor (paper-sim phase):
- Post simulated resting bids one tick inside best bid on chosen legs; log book-state at post time.
- Fill model: conservatively, count a simulated fill only when the printed trade price crosses our level (trade-through), not when quotes touch it — avoids optimistic queue assumptions since FIFO priority is only probable, not documented.
- **Markout logging is the point**: for every simulated fill, record mid at t+10s/60s/300s/1800s. This produces the AS(q) distribution nobody has published. Decision rule after ≥2 weeks: go live iff median AS < (1+ρ)·r·q(1−q) + captured half-spread on the target legs.
- Legging state machine: PARTIAL sets either (a) continue working remaining legs with edge re-check per sweep, (b) convert via NegRiskAdapter (re-read getFeeBips on-chain before each session — currently 0, verified, but contract-settable), or (c) unwind maker-side. Hard timeout → taker-complete only if residual edge after taker fees > 0.
- Ops: handle matching-engine restarts (re-post logic), track rebate accrual via the maker-rebates API endpoint, respect the 5-decimal fee rounding when sizing tiny tail fills.

### 2. Tails-heavy ladder targeting (cheap, ship into existing sweeps)
...
### 3. Fee-free geopolitics watchlist (cheap, immediate)
...
### 4. Venue expansion (after 1–3 are producing data)
...
- **"Finance markets rebate 50% to makers"** (HTX article) — refuted: official docs say 25% for Finance; HTX gives no percentages and also misstates the rebate currency (pUSD, not USDC). Use official docs only.
- **"Taker Rebate Program has 7 tiers"** — refuted by the docs: 6 tiers (Bronze through Obsidian). Corrected above.
- **Kalshi Klear funding requirement for APY** — not present in the cited help article; treat as refuted until sourced.
- **igamingbusiness citation for the July sports fee hike** — the cited URL is a March article about the March 30 change; the 0.03→0.05 hike is real (docs show 0.05) but its July timing rests only on aggregator sites. Date unverified.
- **ForecastEx ladders as neg-risk-equivalent mutually-exclusive sets** — flagged questionable in verification; ForecastEx contracts are typically above/below threshold pairs. Verify per-contract before porting set-arb logic.
- **Kalshi primary fee PDF** — unreachable from this network (TLS cert served for an unrelated domain); all Kalshi numbers are convergent-secondary. Re-verify before building on them. Ceil-vs-round rounding unresolved.
- **Robinhood/Rothera commission schedule** — two mutually exclusive versions circulate (flat $0.01 vs 10%·p(1−p)); unresolved, check the primary PDF.
- **Cross-venue Kalshi–Polymarket sports profitability** ($14–17/1,000 contracts net) — single unverifiable blog; a hypothesis to measure.
- **New-listing-window edge** — blog-only; one blog attributes a 12.3s→2.7s arb-window shrinkage to an academic study that does not contain it. Unmeasured.
- **"Automated strategies net 8–12% annually after fees"** — unsourced aggregator figure; ignore.
- **Kalshi promotional 0% fees / zero-fee crypto perps** — low-quality SEO sources that elsewhere misstate Polymarket's fees; unconfirmed.
- **Composition of our 2,292 fee-free markets** as geopolitics cross-tags — plausible reconciliation, unverifiable publicly; testable directly against Gamma tag data.
- **Maker migration (vs pre-existing concentration) in the rebate leaderboard** — inference the data cannot prove.
- **Fee denomination side** (tokens vs USDC on buys/sells) — value-neutral to second order but unconfirmed for the 2026 regime; check against one live fill during sim.
- **Adverse-selection magnitudes on Polymarket resting orders** — no published numbers exist; this is precisely what build 1's markout logging produces.
